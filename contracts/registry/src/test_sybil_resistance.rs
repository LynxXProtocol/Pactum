...
fn test_sybil_wash_trading_resistance_single_counterparty() {
    ...
    for i in 0..10u64 {
        ...
        let id = client.create_commitment(&issuer, &accomplice, &terms, &(2000 + i), &0);
...
fn test_diverse_high_value_commitments_increase_score() {
    ...
    for i in 0..5u64 {
        ...
        let terms = BytesN::from_array(&env, &[(i as u8); 32]);
        let id = client.create_commitment(&issuer, &counterparty, &terms, &(2000 + i), &1_000_000_000);
        client.attest(&issuer, &id, &CommitmentStatus::Fulfilled);
    }
...
fn test_value_weighting_micro_vs_macro() {
    ...
    let id1 = client.create_commitment(&issuer, &counterparty1, &terms1, &2000, &0);
    ...
    let id2 = client.create_commitment(&issuer, &counterparty2, &terms2, &2000, &10_000_000_000);
